...
Patch numbers advance by exactly one per release: `0.1.1` → `0.1.2` →
`0.1.3`. Never skip or jump patch numbers (`0.1.1` → `0.1.4` is wrong), even
...
gemchain guard ask-monitoring
gemchain update ask-monitoring <new-version> --dry-run
gemchain update ask-monitoring <new-version>
```
...
3. Build passes — `bundle exec rake build` (or
   `gem build ask-monitoring.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/monitoring/version.rb` by gemchain, not by
   hand.
...
| `lib/ask/monitoring/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.1.1` → `0.1.2`.
- Several bug fixes ship together: one release, `0.1.1` → `0.1.2` (never
  `0.1.4`).
- New backwards-compatible API: `0.1.2` → `0.2.0`.
...
- This gem bumped because a dependency released: patch only, e.g. `0.1.2` →
  `0.1.3`, via the gemchain cascade — never hand-published.
